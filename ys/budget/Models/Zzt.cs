    ...
    public class Zzt
    {
        public static List<cls中证通订单> 获取中证通订单s(string sWhere)
        {
            ...
            try
            {
                ...
                str1 = "select * from V中证通证书统计 " + sWhere + " limit 0,500";  //Mysql
        ...
        public static string 保存预算收支(string 证书ID, cls中证通订单预算收支 yssz)
        {
            ...
            try
            {
                ...
                adp = new MySqlDataAdapter("select * from A中证通证书预算收支 where 证书ID='" + 证书ID + "'", cnn);
                ds = new DataSet();
                adp.Fill(ds);
                tb = ds.Tables[0];
                bool isnew = true;
        ...
        public static string 保存实际收支(string 证书ID, cls中证通订单实际收支 sjsz)
        {
            ...
            try
            {
                ...
                adp = new MySqlDataAdapter("select * from A中证通证书实际收支 where 证书ID='" + 证书ID + "'", cnn);
                ds = new DataSet();
                adp.Fill(ds);
                tb = ds.Tables[0];
                bool isnew = true;
        ...
        public static string 保存实际收支批量(List<cls中证通订单> 订单s, cls中证通订单实际收支 sjsz, string 收入支出)
        {
            ...
            try
            {
                ...
                MySqlDataAdapter adp1 = new MySqlDataAdapter("select * from A中证通证书实际收支 where 1=2", cnn);
