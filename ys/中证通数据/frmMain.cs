...
namespace 中证通数据
{
    public partial class frmMain : Form
    {
        ...
        private void b仅获取_Click(object sender, EventArgs e)
        {
            ...
            str1 = "select '' as 导入失败原因,renke_OrdersID as 订单ID,renke_OrderChildID as 子订单ID,OrdersNums as 订单号,OrderChildNums as 子订单号,MatterID as 公证事项,PnoID as 承办公证处,matterPeoper as 当事人,matterPeoperCard as 当事人证件号,CreateTime as 订单日期,CountryArea_Name as 用地,Language_Name as 语种,IsUrgent as 是否加急,renke_UserID as 销售,NotaryFees as 公证费,NotaryFees_Urgent as 加急费,NotaryTranslateFees_old as 证词翻译费,NotaryTranslateFees as 证词翻译费_支出,NotaryTranslateFees_power as 证词翻译费_支出方,AffixTranslateFees_old as 附件翻译费,AffixTranslateFees as 附件翻译费_支出,AffixTranslateFees_power as 附件翻译费_支出方,OfficialAuthFees_0 as 官方认证费,OfficialAuthFees_1 as 外办认证费,ServiceAuthFees as 服务认证费,CopyCost as 副本费,ServiceFees as 渠道服务费,ServiceFeesRenke as 人科服务费,ResearchFees as 调查费,TranslateServerFees as 翻译服务费,DiscountFees as 优惠金额,SetFees as 退回费用,ChannelID as 渠道 ";
            str1 = str1 + " from V预算获取 order by CreateTime";
        ...
        private void b立即获取_Click(object sender, EventArgs e)
        {
            ...
            for (i = 0; i < grid.Rows.Count; i++)
            {
                ...
                try { s当事人 = grid.Rows[i].Cells["当事人"].Value.ToString(); }
                catch { }
                try { s当事人证件号 = grid.Rows[i].Cells["当事人证件号"].Value.ToString(); }
                catch { }
                try { s订单日期 = DateTime.Parse(grid.Rows[i].Cells["订单日期"].Value.ToString()); }
                catch { }
                try { s公证事项 = grid.Rows[i].Cells["公证事项"].Value.ToString(); }
                catch { }
                try { s用地 = grid.Rows[i].Cells["用地"].Value.ToString(); }
                catch { }
                //try { s用途 = grid.Rows[i].Cells["用途"].Value.ToString(); }
                //catch { }
                try { s语种 = grid.Rows[i].Cells["语种"].Value.ToString(); }
                catch { }
                try { s承办公证处 = grid.Rows[i].Cells["承办公证处"].Value.ToString(); }
                catch { }
                ...
                if ((s是否加急 == "") | (s是否加急 == "0"))
                    ...
                    s是否加急 = "是";
                try { s销售 = grid.Rows[i].Cells["销售"].Value.ToString(); }
                catch { }
                ...
                try { s渠道 = grid.Rows[i].Cells["渠道"].Value.ToString(); }
                catch { }
                try { s证词翻译费_支出方 = grid.Rows[i].Cells["证词翻译费_支出方"].Value.ToString(); }
                catch { }
                try { s附件翻译费_支出方 = grid.Rows[i].Cells["附件翻译费_支出方"].Value.ToString(); }
                catch { }

                str1 = "insert into A中证通证书(ID,订单号,订单ID,子订单号,子订单ID,当事人,当事人证件号,订单日期,公证事项,用地,用途,语种,承办公证处,是否加急,销售,渠道,导入日期,备注) values(";
                str1 = str1 + "'" + 证书ID.ToString() + "',";
                str1 = str1 + "'" + s订单号 + "',";
                str1 = str1 + "'" + s订单ID + "',";
                str1 = str1 + "'" + s子订单号 + "',";
                str1 = str1 + "'" + s子订单ID + "',";
                ...
                {
                    int n = 0;
                    n = insertyssz(cnn, 证书ID, "公证费", "支出", s承办公证处, s公证费, s导入日期, "");   //公证费是支出
                    n = insertyssz(cnn, 证书ID, "加急费", "收入", "中证通", s加急费, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "证词翻译费", "收入", "中证通", s证词翻译费, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "证词翻译费", "支出", s证词翻译费_支出方, s证词翻译费_支出, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "附件翻译费", "收入", "中证通", s附件翻译费, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "附件翻译费", "支出", s附件翻译费_支出方, s附件翻译费_支出, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "官方认证费", "收入", "中证通", s官方认证费, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "外办认证费", "收入", "中证通", s外办认证费, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "服务认证费", "收入", "中证通", s服务认证费, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "副本费", "支出", s承办公证处, s副本费, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "渠道服务费", "支出", s渠道, s渠道服务费, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "人科服务费", "收入", "中证通", s人科服务费, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "调查费", "收入", "中证通", s调查费, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "翻译服务费", "收入", "中证通", s翻译服务费, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "优惠金额", "支出", s当事人, s优惠金额_支出, s导入日期, "");
                    n = insertyssz(cnn, 证书ID, "退回费用", "支出", s当事人, s退回费用_支出, s导入日期, "");
                ...
                else
                {
                    ...
                    grid.Rows[i].Cells["导入失败原因"].Value = sErr;
                    ...
                    {
                        int n1 = CommonFunctions.ValInt(Program.获取一般数据集mysql("select count(*) from A中证通证书 where 订单ID='" + s订单ID + "'").Rows[0][0].ToString());
                        int n2 = CommonFunctions.ValInt(Program.获取一般数据集mysql("select count(*) from V预算获取 where renke_OrdersID='" + s订单ID + "'").Rows[0][0].ToString());
        ...
        private int insertyssz(MySqlConnection cnn, Guid zsid, string lx, string shouzhi, string jdf, decimal je, DateTime daoruriqi, string bz)
        {
            if (je != 0.00m)
            {
                string str1 = "";
        ...
        private int insertjili(MySqlConnection cnn, Guid zsid, string lx, string jdf, decimal je, DateTime daoruriqi)
        {
            int i = 0;
        ...
        {
            if (MessageBox.Show("确认要设置中证通的全部数据为未导入状态？如果这样设置，系统会尝试所有数据重新导入预算系统（预算系统已存在的数据不尝试覆盖）。", "询问", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No) return;
